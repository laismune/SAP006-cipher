    ...
    for (var i = 0; i < string.length; i++) {
      ...
      if (offset <= 0) {
        ...
        if (codeAsc <= 90 && codeAsc >=65) {
          ...
          let newAlphabetIndex = (alphabetIndex + offset) % 26;
  ...
  decode: function (string, offset) {
    ...
    for (var i = 0; i < string.length; i++) {
      ...
      if (offset <= 0) {
        ...
        if (codeAsc <= 90 && codeAsc >=65) {
          if (codeAsc - offset >= 65) {
            let newAlphabetIndex = codeAsc - offset;
            let newLetter = String.fromCharCode(newAlphabetIndex);
            messageDecoded += newLetter;		
          } else {
            let newAlphabetIndex = codeAsc - offset + 26;
            let newLetter = String.fromCharCode(newAlphabetIndex)
            messageDecoded += newLetter
          }			
        } 
        else if (codeAsc <= 122 && codeAsc >= 97) {
          if (codeAsc - offset >= 97) {
            let newAlphabetIndex = codeAsc - offset;
            let newLetter = String.fromCharCode(newAlphabetIndex);
            messageDecoded += newLetter;		
          } else {
